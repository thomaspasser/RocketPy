...
class Flight:
    ...
    def uDotParachute(self, t, u, postProcessing=False):
        ...
        if postProcessing:
            ...
            # Atmospheric Conditions
            self.windVelocityX.append([t, self.env.windVelocityX(z)])
            self.windVelocityY.append([t, self.env.windVelocityX(z)])
            self.density.append([t, self.env.density(z)])
            self.dynamicViscosity.append([t, self.env.dynamicViscosity(z)])
            self.pressure.append([t, self.env.pressure(z)])
            self.speedOfSound.append([t, self.env.speedOfSound(z)])
    ...
    def postProcess(self):
        ...
        # Euler Angles
        psi = (180 / np.pi) * (np.arctan2(self.e3[:, 1], self.e0[:, 1]) + np.arctan2(-self.e2[:, 1], -self.e1[:, 1])) # Precession angle
        psi = np.column_stack([self.e1[:, 0], psi])  # Precession angle
        self.psi = Function(psi, "Time (s)", "Precession Angle - ψ (°)")

        phi = (180 / np.pi) * (np.arctan2(self.e3[:, 1], self.e0[:, 1]) - np.arctan2(-self.e2[:, 1], -self.e1[:, 1])) # Spin angle
        phi = np.column_stack([self.e1[:, 0], phi])  # Spin angle
        self.phi = Function(phi, "Time (s)", "Spin Angle - φ (°)")

        theta = (180 / np.pi) * 2 * np.arcsin(-(self.e1[:, 1] ** 2 + self.e2[:, 1] ** 2)**0.5)  # Nutation angle
        ...
        outOfRailTimeIndex = np.searchsorted(F11[:, 0], self.outOfRailTime) # Find out of rail time index
        ...
        self.maxRailButton1NormalForce = np.amax(self.railButton1NormalForce[:outOfRailTimeIndex])
        self.maxRailButton1ShearForce = np.amax(self.railButton1ShearForce[:outOfRailTimeIndex])
        self.maxRailButton2NormalForce = np.amax(self.railButton2NormalForce[:outOfRailTimeIndex])
        self.maxRailButton2ShearForce = np.amax(self.railButton2ShearForce[:outOfRailTimeIndex])
        ...
        for i in range(len(self.attitudeVectorX[:, 1])):
            ...
            if self.freestreamSpeed[i, 1] < 1e-6:
                ...
                dotProductNormalized = 1 if dotProductNormalized > 1 else dotProductNormalized
                dotProductNormalized = -1 if dotProductNormalized < -1 else dotProductNormalized
